import { SignalPoint } from './models';
...
export interface MathDefinition {
  ...
  autoEnableSignals?: string[];
  autoLoad?: MathAutoLoad;
...
/** Port of legacy/src/mathdefinitions.js — the derived-signal formula catalog. */
export const MATH_DEFINITIONS: MathDefinition[] = [
  {
  ...
  },
  {
    ...
    customProcess: (signals) => {
      ...
      const result: Array<SignalPoint & { _rawV: number }> = [];
      const toRad = (val: number) => (val * Math.PI) / 180;
      const R = 6371e3;
      ...
      for (let i = 1; i < latSig.length; i++) {
        ...
        if (i > 1) {
          ...
          result.push({ x: t2, y: gForce, _rawV: v });
        } else {
  ...
  },
  {
    ...
    inputs: [
      { name: 'Fuel Level', label: 'Fuel Level (%)' },
      {
        name: 'capacity',
    ...
    customProcess: (signals, constants) => {
      ...
      return source.map((point) => {
        ...
        return { x: point.x, y: consumedLiters * price };
      });
    },
  },
  {
  ...
  },
  {
    ...
    inputs: [
      { name: 'Fuel Level', label: 'Fuel Level (%)' },
      { name: 'Fuel Consumption', label: 'Current Cons. Signal' },
      {
        name: 'capacity',
        label: 'Tank Capacity (Liters)',
        isConstant: true,
    ...
    formula: (values) => {
      const pct = Math.min(Math.max(values[0], 0), 100);
      const cons = Math.max(values[1], 0.1);
      const cap = values[2];
  ...
  },
  {
    ...
    inputs: [{ name: 'Distance', label: 'Odometer Signal' }],
    customProcess: (signals) => {
      const source = signals[0];
      if (!source || source.length === 0) return [];
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: 'Engine Speed', label: 'Engine RPM' },
      {
        name: 'factor',
  ...
  },
  {
    ...
    inputs: [{ name: 'Vehicle Speed', label: 'Speed (km/h)' }],
    customProcess: (signals) => {
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: 'Atmospheric Pressure', label: 'Atmospheric Pressure' },
    ],
    formula: (values) => values[0] - values[1],
  },
  {
    ...
    inputs: [
      { name: 'AFR Commanded', label: 'AFR Commanded' },
      { name: 'AFR Measured', label: 'AFR Measured' },
    ],
    formula: (values) => values[0] - values[1],
  },
  {
  ...
  },
  {
    ...
    inputs: [
      ...
      },
      {
        ...
        defaultValue: '1',
        options: [
          { value: '1', label: 'Pass if Condition > Threshold (High Pass)' },
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: 'thresh', label: 'Threshold', isConstant: true, defaultValue: 0 },
      {
        name: 'mode',
        label: 'Mode',
        isConstant: true,
    ...
    inputs: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: '0', label: 'Pass if Outside Range (X < Min OR X > Max)' },
        ],
      },
      {
        name: 'fallback',
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: 'factor', label: 'Factor', isConstant: true, defaultValue: 1.0 },
    ],
    formula: (values) => values[0] * values[1],
  },
  // --- GAS PEDAL FILTERING ---
  {
    ...
    inputs: [
      { name: 'sources', label: 'Signals to Filter', isMulti: true },
      {
        name: [
          'Gas Pedal Position',
          'Gas Pedal',
      ...
      { name: 'mode', label: 'Mode', isConstant: true, defaultValue: '1' },
      {
        name: 'fallback',
        label: 'Fallback Value',
        isConstant: true,
